...
class Control:
    ...
    def Obey_RightTurn(self, Angle, Speed, Mode, Tracked_class):
        if Tracked_class == "right_turn":  # 우회전이 감지된 경우
            ...
            if self.prev_Mode_LT == "Detection" and Mode == "Tracking":
                ...
                if self.Right_turn_iterations % 10 == 0 and self.Right_turn_iterations > 40:
                    self.Frozen_Angle = self.Frozen_Angle + 34  # 우회전 각도 추가
                if self.Right_turn_iterations == 120:  # 우회전 종료 조건
                    self.prev_Mode_LT = "Detection"
                    self.Activat_RightTurn = False
                    self.Right_turn_iterations = 0
                    self.Frozen_Angle = 0
    ...
    def drive_car(self,Current_State,Inc_TL,Inc_LT):
        ...
        return self.angle_of_car,current_speed, Detected_RightTurn, Activat_RightTurn
...
class Car:
    ...
    #     angle = 0.0
    #     speed = 0.0
    #     self.control.publish_control_commands(angle, speed)  # 멈춤 명령 발행
    ...
    def driveCar(self,frame):
        ...
        Angle,Speed, Detected_RightTurn, Activat_RightTurn  = self.Control_.drive_car(Current_State,self.Inc_TL,self.Inc_LT)
