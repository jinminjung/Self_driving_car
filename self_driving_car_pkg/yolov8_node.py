...
class Yolov8Node(Node):
    ...
    def __init__(self) -> None:
        ...
        # Parameters
        self.declare_parameter("model", '/home/jinmin/auto_ws/src/self_driving_car_pkg/self_driving_car_pkg/simcar_append_best.pt') # your trained model, default yolov8m.pt
    ...
    def image_cb(self, msg: Image) -> None:
        if self.enable:
            ...
            for b in results.boxes:
                ...
                x_goal, y_goal = width // 2, height // 2
                distance = np.sqrt((x_goal - detection.bbox.center.x) ** 2 + (y_goal - detection.bbox.center.y) ** 2)
                bbox_size = detection.bbox.size_x * detection.bbox.size_y
                rate = distance / bbox_size
                self.get_logger().info(f"rate: {rate:.4f}")

                # Object detect    
                if label == "person" and rate < 0.035:
                    person_detected = True
                    self.get_logger().info("Person detected!")
                ...
                detection_str = "[ detected ] = [ person => " + str(person_detected)+ "]"
                cv2.putText(cv_image,str(detection_str),(20,40),font, 1.3,(0,0,255),2)
            ...
            if person_detected:
                ...
                self.car_control.publish_control_commands(angle, speed)
