...
from .Drive_Bot_Navi import Car, Debugging
...
class Video_feed_in(Node):
    def __init__(self):
        ...
        # bg_model 초기화
        self.navigator.bot_localizer.bg_model = None  # Navigator를 통해 bot_localizer 초기화
        ...
        # Container to store frame retrieved from overhead_view
        self.sat_view = []

        # [NEW]: Subscribing to receive the robot pose in simulation
    ...
    def process_data(self, data): 
        ...
        # bg_model 초기화 검사 및 설정
        if not hasattr(self.navigator.bot_localizer, 'bg_model') or self.navigator.bot_localizer.bg_model is None:
            self.navigator.bot_localizer.bg_model = np.zeros_like(frame)
